...
singularity instance start -B /dev/shm:/run/shm -B $(readlink -f $SANDBOX) -B $(readlink -f $PWD) -B $(readlink -f .${SERVER}/lib):/opt/tritonserver/lib -B $(readlink -f models):/models ${SANDBOX}/ ${SERVER}
